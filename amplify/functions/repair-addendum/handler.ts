...
// field_value uses LONGTEXT (vs. worksheet's TEXT) since photo/invoice
// uploads are persisted as base64 data URLs, which can exceed 64KB.
const CREATE_TABLE_SQL = `
  CREATE TABLE IF NOT EXISTS repair_addendum_fields (
  ...
  try {
    ...
    if (method === "GET") {
      ...
      const [rows] = await getPool().execute<mysql.RowDataPacket[]>(
        "SELECT field_name, field_value FROM repair_addendum_fields WHERE project_uuid = ?",
        [projectId]
      );
    ...
    if (method === "POST") {
      ...
      for (const [fieldName, fieldValue] of entries) {
        await db.execute(
          `INSERT INTO repair_addendum_fields (project_uuid, field_name, field_value)
           VALUES (?, ?, ?)
